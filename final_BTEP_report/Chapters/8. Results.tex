...
\subsection{Offline PPO Model Results}
...
\subsection{End-to-End KPI Comparison}
...
\begin{figure}[H]
    ...
    \caption{WebUI dashboard view during campaign execution}
    \label{fig:webui_dashboard}
\end{figure}

\textbf{Explanation of Figure \ref{fig:webui_dashboard}:} The dashboard view provides a consolidated status of the active scheduler, worker availability, and task lifecycle progression. This helped verify that PPO placement decisions were not only better in aggregate metrics, but also operationally consistent while the workload was running.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../diagrams/plot.jpg}
    \caption{WebUI trend plot for scheduler performance tracking}
    \label{fig:webui_plot}
\end{figure}

\textbf{Explanation of Figure \ref{fig:webui_plot}:} The trend plot visualizes runtime behavior across the campaign window, making it easier to inspect response under pressure and detect tail effects that may be hidden in simple averages. This visual evidence aligns with the measured improvements in turnaround and P95 completion for PPO in contention-heavy scenarios.
