...
List files generated by the authenticated user only.

**Notes:**
- `user_id` and `requesting_user` query parameters are no longer accepted.
- Supplying legacy identity query parameters returns `400 Bad Request`.
...
Get files for a specific task. Task owner is resolved server-side from task metadata.
...
Download a specific output file for a task (requires authentication and ownership/break-glass authorization).

---

**Admin Break-Glass Result Access**

The following endpoints are admin-only and require `X-Breakglass-Reason` for cross-user result reads:
...
- **Role model:** `user` and `admin` roles in `USERS` collection
- **Admin bootstrap:** `ADMIN_EMAILS` environment variable
...
**Result Access Control (Hard Cutover):**

- Task owners can read only their own task details, logs, and generated files.
- Admins can perform cluster operations globally (worker/task control-plane actions).
- Cross-user result reads by admins require explicit break-glass (`X-Breakglass-Reason`) and are audit logged.
- Legacy identity query/body controls (`user_id`, `requesting_user`) are rejected for result APIs.

**Deferred Security Work:**
- Worker↔Master transport hardening (mutual TLS + worker authentication)
- API key management for programmatic access
- User quotas and resource limits

See [docs/RESULT_ACCESS_SECURITY.md](RESULT_ACCESS_SECURITY.md) for the complete result-visibility boundary and break-glass flow.
